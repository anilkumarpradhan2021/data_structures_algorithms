...
def fetch_latest_news_from_scraping(url):
    ...
    try:
        ...
        for tag in headline_tags:
            for element in soup.find_all(tag):
                ...
                if headline:  # Ensure it's not empty
                    ...
                    description = None
                    description_tag = element.find_next(
                        "p"
                    )  # Find next <p> tag after headline
                    if description_tag:
                        description = description_tag.get_text(strip=True)

                    # Find the URL (if the headline is a link)
                    article_link = element.find_parent("a", href=True)
                    if article_link:
                        article["url"] = urljoin(
                    ...
                    article["description"] = description if description else headline

                    # Try to find images related to the article
                    images = get_images_from_article(soup, url)
        ...
        if not articles:
            ...
            for a in soup.find_all("a", href=True):
                ...
                ):  # Check for longer text to avoid short links
                    article = {
                        "title": text,
        ...
        for article in articles[1:]:
            ...
            result = fetch_content_and_images_from_article(article_url, headers)
            # article["urlToImage"] = fetch_images_from_article(article_url, headers)
            article["description"] = result["content"]
            article["urlToImage"] = result["images"]

        # Display results (only first 10 articles)
        if articles:
            print("\nLatest News Headlines and More Content:")
            for i, article in enumerate(articles[1:]):  # Display top 2 articles
    ...
    # Set to keep track of seen article titles
    seen_titles = set()

    # New list to store unique articles
    unique_articles = []
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            article_section = (
                ...
                or article_soup.find("main")
                or article_soup.find("section")
            )
            ...
            for img_tag in article_section.find_all("img", src=True):
                ...
                if any(src.lower().endswith(ext) for ext in valid_extensions):
                    ...
                    if "header" in img_tag.get("class", []) or "footer" in img_tag.get(
                        "class", []
                    ):
                        continue
                    ...
                    try:
                        ...
                        if img_response.status_code == 200:
                            ...
                            if img_width > 0 and img_height > 0:
                                ...
                                image_sizes[img_url] = (
                                    image_size  # Store image URL with its size
                                )
...
def fetch_content_and_images_from_article(article_url, headers):
    content = []
    image_urls = set()  # Using set to ensure uniqueness
    valid_extensions = [
        ".jpg",
        ".jpeg",
        ".png",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Try to find the main content section (article, main, or section)
            article_section = (
                article_soup.find("article")
                or article_soup.find("main")
                or article_soup.find("section")
            ...
            # 1. Fetch textual content from <p> tags
            for p_tag in article_section.find_all("p"):
                content_text = p_tag.get_text(strip=True)
                if content_text:
                    content.append(content_text)

            # 2. Fetch image URLs and their sizes
            for img_tag in article_section.find_all("img", src=True):
                src = img_tag.get("src")
                if not src:
                    continue
            ...
            # Now, find the maximum size image(s) based on image area
            if image_sizes:
                max_size = max(image_sizes.values())
                # Filter images that have the maximum size
                for img_url, size in image_sizes.items():
                    if size == max_size:
                        image_urls.add(img_url)  # Add to set to ensure uniqueness

        else:
            return {"content": [], "images": [], "error": "Failed to fetch article."}

    except requests.exceptions.RequestException as e:
        print(f"Error fetching the article: {e}")
        return {"content": [], "images": [], "error": str(e)}

    # Return both content and images in a dictionary
    return {"content": content, "images": list(image_urls)}
...
def get_images_from_article(soup, base_url):
    ...
    for img_tag in soup.find_all("img"):
        ...
        if src:
            ...
            img_width = convert_to_numeric(img_tag.get("width", 0))
            img_height = convert_to_numeric(img_tag.get("height", 0))

            if img_width and img_height:
                img_width = int(float(img_width))
                img_height = int(float(img_height))
                if (
                    img_width > 100 and img_height > 100
                ):  # Filter out tiny images (e.g., thumbnails)
...
time.sleep(
    random.randint(1, 3)
)  # Random sleep time between requests (to prevent rate-limiting)
